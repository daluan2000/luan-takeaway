...
### 1.1 登录请求构造
- 文件：`luan-ui/src/stores/userInfo.ts`
- 方法：`useUserInfo.actions.login(data)`
  - 设置 `data.grant_type = 'password'`、`data.scope = 'server'`
  - 调用 `luan-ui/src/api/login/index.ts` 的 `login(data)`

- 文件：`luan-ui/src/api/login/index.ts`
- 方法：`login(data)`
...
### 2.1 安全链与 token 端点入口
- 文件：`luan-auth/src/main/java/com/pig4cloud/pig/auth/config/AuthorizationServerConfiguration.java`
...
- 文件：`luan-auth/.../OAuth2ResourceOwnerPasswordAuthenticationConverter.java`
- 方法：
...
### 2.3 用户认证（账号密码校验）
- 文件：`luan-auth/.../OAuth2ResourceOwnerBaseAuthenticationProvider.java`
...
- 文件：`luan-auth/.../PigDaoAuthenticationProvider.java`
- 方法：
...
- 文件：`luan-ui/src/utils/request.ts`
...
### 4.1 启用资源服务器
- 文件：`luan-common/luan-common-security/src/main/java/com/pig4cloud/pig/common/security/annotation/EnablePigResourceServer.java`
...
### 5.3 业务接口示例
- 文件：`luan-upms/luan-upms-biz/src/main/java/com/pig4cloud/pig/admin/controller/SysUserController.java`
- 方法：
